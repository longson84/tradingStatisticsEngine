...
function computePeriods(curve: Record<string, number>): DDPeriod[] {
    ...
    if (val >= peak) {
      if (inDD) {
        ...
        if (depth < -DD_MIN_THRESHOLD)
          periods.push({
            daysToTrough: calDays(startDate, troughDate),
            recoveryDays: calDays(troughDate, date),
          })
        inDD = false
      }
      peak = val
  ...
  if (inDD) {
    const depth = (troughVal / startPeak - 1) * 100
    if (depth < -DD_MIN_THRESHOLD)
      periods.push({ daysToTrough: calDays(startDate, troughDate), recoveryDays: null })
  }
  return periods
}
...
// Blue heat for strategy, orange heat for BAH
function stratHeat(val: number): React.CSSProperties {
  const saturateAt = 365
  const intensity = Math.min(val / saturateAt, 1)
  return {
...
function bahHeat(val: number): React.CSSProperties {
  ...
  const intensity = Math.min(val / saturateAt, 1)
  return {
    color: intensity > 0.5 ? "#c2410c" : "#ea580c",
        ...
        <table className="w-full text-xs tabular-nums border-collapse">
          <thead>
            <tr className="border-b border-border text-muted-foreground uppercase tracking-wide">
              <th className="py-2 px-3 text-left font-medium">Duration (days)</th>
          ...
          <tbody>
            {DURATION_PCTS.map(p => {
              const sv = pctile(stratDays, p)
              const bv = pctile(bahDays, p)
...
const DUR_BUCKETS = [
  { label: "<30d",    min: 0,   max: 30,       stratColor: "#60a5fa", bahColor: "#fdba74" },
  { label: "30–90d",  min: 30,  max: 90,       stratColor: "#3b82f6", bahColor: "#fb923c" },
  { label: "90–180d", min: 90,  max: 180,      stratColor: "#2563eb", bahColor: "#f97316" },
  { label: "180–365d",min: 180, max: 365,      stratColor: "#1d4ed8", bahColor: "#ea580c" },
  { label: ">365d",   min: 365, max: Infinity, stratColor: "#1e40af", bahColor: "#c2410c" },
]
...
function DurationHistogram({
  ...
  const counts = DUR_BUCKETS.map(b => ({
    ...b,
  ...
  const barPad = 3
  const barH = (rowH - barPad * 3) / 2  // two bars per row
    ...
    <div className="relative">
          ...
          {counts.map((b, i) => {
            ...
            return (
              <g key={b.label}>
                ...
                {/* Bucket label centered between the two bars */}
      ...
      {tooltip && (
        ...
        >
          ...
          <div className="text-muted-foreground">
            Buy &amp; Hold: <span className="font-medium text-orange-400">{tooltip.bah}</span>
          </div>
        </div>
      )}
      ...
      <div className="mt-1.5 flex gap-4 text-xs text-muted-foreground px-1">
        ...
        <span className="flex items-center gap-1.5">
          <span className="inline-block w-2.5 h-2.5 rounded-sm opacity-80 bg-orange-400" />
          Buy &amp; Hold
        </span>
      </div>
    </div>
...
function DurationVsRecovery({
  ...
  const stratPoints: ScatterPoint[] = stratPeriods
    ...
    .map(p => ({ duration: p.daysToTrough, recovery: p.recoveryDays!, isStrat: true }))

  const bahPoints: ScatterPoint[] = bahPeriods
    .filter(p => p.recoveryDays != null && p.recoveryDays > 0)
    .map(p => ({ duration: p.daysToTrough, recovery: p.recoveryDays!, isStrat: false }))
  ...
  const maxDur = Math.max(...allPoints.map(p => p.duration)) * 1.08
  const maxRec = Math.max(...allPoints.map(p => p.recovery)) * 1.08

  const xs = (v: number) => (v / maxDur) * IW
  const ys = (v: number) => IH - (v / maxRec) * IH

  const xTicks = [0, 0.25, 0.5, 0.75, 1].map(f => Math.round(f * Math.max(...allPoints.map(p => p.duration))))
  ...
  function handleMouseMove(e: React.MouseEvent<SVGSVGElement>) {
    ...
    for (const p of allPoints) {
      const d = Math.hypot(svgX - xs(p.duration), svgY - ys(p.recovery))
  ...
  return (
    <div className="relative">
      <h2 className="text-[11px] font-semibold uppercase tracking-widest text-muted-foreground mt-1 mb-3">
        Duration vs Recovery — Closed Drawdowns
      </h2>
      <svg
        viewBox={`0 0 ${W} ${H}`}
        className="w-full cursor-crosshair"
      ...
      >
        <g transform={`translate(${MX.left},${MX.top})`}>
          ...
          {bahPoints.map((p, i) => (
            <circle key={`b${i}`}
              cx={xs(p.duration)} cy={ys(p.recovery)} r={4}
              fill="#fb923c" fillOpacity={0.65}
              stroke="#ea580c" strokeWidth={0.8}
            />
          ))}
          ...
          {tooltip && (
            <circle
              cx={xs(tooltip.point.duration)} cy={ys(tooltip.point.recovery)} r={6}
              fill="none"
              stroke={tooltip.point.isStrat ? "#3b82f6" : "#ea580c"}
              strokeWidth={2}
            />
          )}
          ...
          <text x={IW / 2} y={IH + 40} textAnchor="middle" fontSize={11}
            fill="hsl(var(--foreground))" fontWeight="500">Days to Trough</text>
      ...
      {tooltip && (
        ...
        >
          <div className="font-semibold mb-1"
            style={{ color: tooltip.point.isStrat ? "#60a5fa" : "#fb923c" }}>
            {tooltip.point.isStrat ? strategyLabel : "Buy & Hold"}
          </div>
          <div className="text-muted-foreground">
            To trough: <span className="text-foreground font-medium">{tooltip.point.duration}d</span>
          </div>
          <div className="text-muted-foreground">
      ...
      <div className="mt-1.5 flex gap-4 text-xs text-muted-foreground px-1">
        ...
        <span className="flex items-center gap-1.5">
          <span className="inline-block w-2.5 h-2.5 rounded-full bg-orange-400 opacity-70" />
          Buy &amp; Hold
        </span>
