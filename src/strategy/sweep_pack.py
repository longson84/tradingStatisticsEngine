...
from src.strategy.strategies import STRATEGY_NAMES, STRATEGY_REGISTRY
...
class ParameterSweepPack(StrategyBacktestPack):
    ...
    def render_sidebar(self) -> Dict[str, Any]:
        ...
        config = STRATEGY_REGISTRY[strategy_type].sweep_sidebar(ticker, data_source)
    ...
    def _build_strategy(self, config: Dict, length):
        """Build a strategy instance for one sweep length."""
        return STRATEGY_REGISTRY[config["strategy_type"]].build_from_sweep_config(config, length)

    def _make_label(self, config: Dict, length) -> str:
        """Build a short legend label for a sweep variant."""
        return STRATEGY_REGISTRY[config["strategy_type"]].sweep_label(config, length)
    ...
    def run_sweep(
        ...
        for length in config["sweep_lengths"]:
            if STRATEGY_REGISTRY[config["strategy_type"]].should_skip_sweep_length(config, length):
