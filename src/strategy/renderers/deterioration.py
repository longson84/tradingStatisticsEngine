...
_ANNUAL_PERCENTILES = [90, 80, 70, 60, 50, 40, 30, 20, 10]
...
def _render_annual_summary(closed: list, _ks: str) -> None:
    ...
    for yr in sorted(year_trades.keys(), reverse=True):
        ...
        row: Dict[str, Any] = {
            ...
            **format_percentile_columns(rets, _ANNUAL_PERCENTILES),
...
# ---------------------------------------------------------------------------
# 2. Rolling N-Trade Performance
# ---------------------------------------------------------------------------

def _render_rolling_performance(closed: list, _ks: str) -> None:
...
def _render_trade_trend(closed: list, _ks: str) -> None:
    ...
    plot_rets = [_to_log(r) for r in rets] if scatter_log else rets
...
def _render_rolling_12m(strat_equity: pd.Series, _ks: str) -> None:
...
def render_deterioration_section(trades, strat_equity: pd.Series, ticker: str, key_suffix: str = "") -> None:
    ...
    st.divider()
    _render_rolling_performance(closed, _ks)
    st.divider()
    _render_trade_trend(closed, _ks)
    st.divider()
    _render_rolling_12m(strat_equity, _ks)
