...
from src.constants import YFINANCE_PRESETS
...
def sidebar_data_source(key_prefix: str) -> str:
    """Data source selectbox (yfinance / vnstock). Returns the chosen source."""
...
def sidebar_ticker_input(
    ...
    *,
    ...
    Args:
        data_source: "yfinance" or "vnstock".
        key_prefix: unique prefix for Streamlit widget keys.
        multi: if True, returns a list of tickers; if False, returns a single ticker string.

    Returns:
        list[str] when multi=True, str when multi=False.
    """
    if multi:
        ...
        # Reset group selection when data source changes
...
def sidebar_from_date(key_prefix: str) -> Optional[date]:
    """Backtest from-date picker. Returns None when left empty."""
    st.sidebar.divider()
