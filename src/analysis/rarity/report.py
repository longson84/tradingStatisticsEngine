...
class ReportGenerator:
    def __init__(self, ticker, signal, df, signal_series, qr_threshold: int = 5):
        self.ticker = ticker
        self.signal = signal
        self.df = df
    ...
    def calculate(self):
        ...
        self.add_info = self.signal.get_additional_info(self.df)

        self.np_stats, self.highlight_p = build_np_stats_summary_table(self.np_stats, self.signal, self.current_status)

        self.time_range_lines = build_report_time_range_info(self.df['Close'])
